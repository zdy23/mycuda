...
// 定义矩阵乘法核函数，使用共享内存和分块计算
// 将矩阵 A 和 B 相乘，结果存储在矩阵 C 中
__global__ void matrix_multiplication_kernel(const float* A, const float* B, float* C, int M, int N, int K, const float scale) {
    ...
    for (int phase = 0; phase < (N + TILE_WIDTH - 1) / TILE_WIDTH; phase++) {
        ...
        for (int c = 0; c < COARSE_FACTOR; c++) {
			...
			// 检查列索引是否越界，如果没有越界则加载 B 的元素到共享内存，否则加载 0
...
// 定义行 softmax 核函数，对输入矩阵的每一行进行 softmax 操作
// 将输入矩阵 input 的每一行进行归一化处理，结果存储在原矩阵中
...
// 用于将矩阵 input 转置，结果存储在矩阵 output 中
