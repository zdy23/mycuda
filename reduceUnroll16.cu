...
__global__ void reduceCompleteUnroll(int *g_idata, int *g_odata,
                                     unsigned int n) {
  constexpr unsigned int UNROLL_FACTOR = 16;
  ...
  if (idx + (UNROLL_FACTOR - 1) * blockDim.x < n) {
    sum += g_idata[idx];
    ...
    sum += g_idata[idx + blockDim.x * 8];
    sum += g_idata[idx + blockDim.x * 9];
    sum += g_idata[idx + blockDim.x * 10];
    sum += g_idata[idx + blockDim.x * 11];
    sum += g_idata[idx + blockDim.x * 12];
    sum += g_idata[idx + blockDim.x * 13];
    sum += g_idata[idx + blockDim.x * 14];
    sum += g_idata[idx + blockDim.x * 15];
  } else {
...
void gpuReduce(int *d_idata, int *d_odata, unsigned int n,
  ...
  unsigned int gridSize = (n + (unsigned long long)blockSize * 16 - 1) /
                          ((unsigned long long)blockSize * 16);
...
int main() {
  ...
  int gridSize = (n + blockSize * 16 - 1) / (blockSize * 16);
